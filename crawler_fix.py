...
def is_valid(url):
    ...
    return bool(parsed.netloc) and bool(parsed.scheme == 'https')
...
def get_all_website_links(url):
    ...
    for a_tag in soup.findAll("a"):
        ...
        if not is_valid(href):
            # not a valid URL
            continue
        if extension := os.path.splitext(parsed_href.path)[1]:
            # check if the link is a document link
            if extension in ['.doc', '.docx', '.pdf', '.pptx', '.ppt', '.xls', '.xlsx', '.csv', '.txt', '.rtf','.zip','.rar',]:
                document_urls.add(href)
                print(f"{YELLOW}[!] Document link: {href}{RESET}")
                continue
            # check if the link is an image link
            if extension in ['.jpg', '.jpeg', '.png', '.svg', '.gif', '.ico', '.webp', '.bmp', '.tiff', '.tif', '.jfif','.mp4','mp3']:
                image_urls.add(href)
                print(f"{YELLOW}[!] Image or Video link: {href}{RESET}")
                continue
        if href in internal_urls:
            # already in the set
            continue
...
# def get_all_website_links(url, timeout=5):
#     """
#     Returns all URLs that is found on `url` in which it belongs to the same website
#     """
#     # all URLs of `url`
...
#             url, headers=headers, timeout=timeout).content, "lxml")
#         for a_tag in soup.findAll("a"):
#             href = a_tag.attrs.get("href")
#             if href == "" or href is None:
#                 # href empty tag
...
#         print(f"{GRAY}[!] Timeout occurred while crawling: {url}{RESET}")
#         broken_urls.add(url)
#     except Exception as e:
#         print(f"{GRAY}[!] Error occurred while crawling: {url}: {e}{RESET}")
#         broken_urls.add(url)
#     return urls
...
def crawl(url, max_urls=10000):
    ...
    params:
        max_urls (int): number of max urls to crawl, default is 100000.
    """
...
if __name__ == "__main__":
    ...
    print("[+] Total URLs:", len(external_urls) +
          len(internal_urls) + len(subdomains) + len(document_urls))
